/**
 * Script para crear la encuesta de Fase 2 del Rollout
...
const SURVEY_SLUG = `fase-2-escalamiento-v1-1-${Date.now()}`;

// Preguntas a incluir (usando IDs del catálogo CADEM)
// Fase 2: Mismas 3 preguntas que Fase 1 para mantener comparabilidad
const QUESTION_IDS = [
  'q_approval',
  'q_economy_personal',
  'q_optimism'
...
async function createSurveyWithQuestions(questions: any[]): Promise<string | null> {
  console.log('📝 Creando encuesta de Fase 2...');
  console.log(`   Nombre: ${SURVEY_NAME}`);
  console.log(`   Slug: ${SURVEY_SLUG}`);
  console.log(`   Sample size: 500`);
  ...
  const surveyData = {
    name: SURVEY_NAME,
    slug: SURVEY_SLUG,
    description: 'Fase 2 del rollout - Escalamiento controlado a 500 agentes con mismas 3 preguntas que Fase 1',
    status: 'draft',
    sample_size: 500,
    ...
    metadata: {
      ...
      baseline_tag: 'fase-2-rollout-v1.1',
      rollout_phase: '2',
      created_for: 'rollout',
      question_count: questions.length,
      sample_method: 'cadem_quotas',
      previous_phase: '1',
      phase_comparison: 'same_questions_as_fase_1'
    }
  };
...
async function main() {
  console.log('🚀 FASE 2 ROLLOUT - Creación de Encuesta\n');
  console.log('   Escalamiento: 100 → 500 agentes');
  console.log('   Mismas 3 preguntas que Fase 1');
  ...
  if (existingSurvey) {
    ...
    if (existingSurvey.hasQuestions) {
      ...
      console.log(`\n➡️  Ejecutar con: npx tsx scripts/rollout/runPhase2Controlled.ts --survey-id=${existingSurvey.id} --sample-size=500`);
  ...
  console.log('📚 Preguntas seleccionadas del catálogo (mismas que Fase 1):');
  ...
  console.log('\n🎉 Encuesta de Fase 2 creada exitosamente!');
  console.log('\n📋 Resumen:');
  ...
  console.log(`   Fase anterior: 1 (comparabilidad mantenida)`);
  console.log('\n➡️  Próximo paso: Ejecutar la encuesta con:');
  console.log(`   npx tsx scripts/rollout/runPhase2Controlled.ts --survey-id=${surveyId} --sample-size=500`);
}
