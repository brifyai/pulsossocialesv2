/**
 * Script para crear la encuesta de Fase 3 del Rollout
 ...
 * 
 * Fase 3: Escalamiento a 1000 agentes, mismas 3 preguntas que Fases 1 y 2
 */
...
// Configuración de la encuesta de Fase 3
const SURVEY_NAME = 'Fase 3 - Escalamiento Final v1.1';
const SURVEY_SLUG = `fase-3-escalamiento-final-v1-1-${Date.now()}`;

// Preguntas a incluir (usando IDs del catálogo CADEM)
// Fase 3: Mismas 3 preguntas que Fases 1 y 2 para mantener comparabilidad
const QUESTION_IDS = [
  'q_approval',
  'q_economy_personal',
  'q_optimism'
...
async function createSurveyWithQuestions(questions: any[]): Promise<string | null> {
  console.log('📝 Creando encuesta de Fase 3...');
  ...
  const surveyData = {
    name: SURVEY_NAME,
    slug: SURVEY_SLUG,
    description: 'Fase 3 del rollout - Escalamiento final a 1000 agentes con mismas 3 preguntas que Fases 1 y 2',
    status: 'draft',
    sample_size: 1000,
    ...
    metadata: {
      ...
      baseline_tag: 'fase-3-rollout-v1.1',
      rollout_phase: '3',
      created_for: 'rollout',
      question_count: questions.length,
      sample_method: 'cadem_quotas',
      previous_phase: '2',
      phase_comparison: 'same_questions_as_fase_1_and_2',
      performance_note: 'Expected ~13-14 min execution time'
    }
  };
...
async function main() {
  console.log('🚀 FASE 3 ROLLOUT - Creación de Encuesta\n');
  console.log('   Escalamiento: 500 → 1000 agentes');
  console.log('   Mismas 3 preguntas que Fases 1 y 2');
  console.log('   Baseline congelada desde Fase 1\n');
  console.log('   ⚠️  NOTA: Tiempo esperado ~13-14 minutos\n');
  ...
  if (existingSurvey) {
    ...
    if (existingSurvey.hasQuestions) {
      ...
      console.log(`   Persist state: ${data?.metadata?.persist_state}`);
      console.log(`   Preguntas: ${data?.questions?.length || 0}`);
      console.log(`\n➡️  Ejecutar con: npx tsx scripts/rollout/runPhase3Controlled.ts --survey-id=${existingSurvey.id} --sample-size=1000`);
  ...
  console.log('📚 Preguntas seleccionadas del catálogo (mismas que Fases 1 y 2):');
  ...
  console.log('\n🎉 Encuesta de Fase 3 creada exitosamente!');
  console.log('\n📋 Resumen:');
  ...
  console.log(`   Fase anterior: 2 (comparabilidad mantenida)`);
  console.log(`   Tiempo esperado: ~13-14 minutos`);
  console.log('\n➡️  Próximo paso: Ejecutar la encuesta con:');
  console.log(`   npx tsx scripts/rollout/runPhase3Controlled.ts --survey-id=${surveyId} --sample-size=1000`);
  console.log('\n⚠️  ADVERTENCIA: La ejecución tomará aproximadamente 13-14 minutos.');
  console.log('   Asegúrate de tener una conexión estable antes de comenzar.');
}
