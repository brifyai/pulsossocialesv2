/**
 * Script para crear la encuesta de Fase 1 del Rollout
...
// Configuración de la encuesta de Fase 1
const SURVEY_NAME = 'Fase 1 - Rollout Controlado v1.1';
const SURVEY_SLUG = `fase-1-rollout-controlado-v1-1-${Date.now()}`;

// Preguntas a incluir (usando IDs del catálogo CADEM)
// Fase 1: 3 preguntas representativas
const QUESTION_IDS = [
  'q_approval',
  'q_economy_personal',
  'q_optimism'
...
async function createSurveyWithQuestions(questions: any[]): Promise<string | null> {
  console.log('📝 Creando encuesta de Fase 1...');
  ...
  const surveyData = {
    name: SURVEY_NAME,
    slug: SURVEY_SLUG,
    description: 'Fase 1 del rollout controlado - Primera ejecución con sampleo por cuotas tipo Cadem',
    status: 'draft',
    sample_size: 100,
    ...
    metadata: {
      ...
      baseline_tag: 'fase-1-rollout-v1.1',
      rollout_phase: '1',
      created_for: 'rollout',
...
async function main() {
  console.log('🚀 FASE 1 ROLLOUT - Creación de Encuesta\n');
  ...
  if (existingSurvey) {
    ...
    if (existingSurvey.hasQuestions) {
      ...
      console.log(`\n➡️  Ejecutar con: npx tsx scripts/rollout/runPhase1Controlled.ts --survey-id=${existingSurvey.id} --sample-size=100`);
  ...
  console.log('\n🎉 Encuesta de Fase 1 creada exitosamente!');
  console.log('\n📋 Resumen:');
  ...
  console.log(`   npx tsx scripts/rollout/runPhase1Controlled.ts --survey-id=${surveyId} --sample-size=100`);
}
